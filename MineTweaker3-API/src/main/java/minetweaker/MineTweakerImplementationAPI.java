...
public class MineTweakerImplementationAPI {
	...
	static {
		...
		minetweakerCommands.put("entities", new MineTweakerCommand("entities", new String[]{"/minetweaker entities", "    Lists all the entities in the game"}, (arguments, player) -> {
			MineTweakerAPI.logCommand("Entities:");
			for(IEntityDefinition entity : MineTweakerAPI.game.getEntities()) {
				MineTweakerAPI.logCommand("    " + entity.getName());
			}
			
			player.sendChat("Entity list generated; see minetweaker.log in your minecraft dir");
		}));
		...
		minetweakerCommands.put("blockinfo", new MineTweakerCommand("blockinfo", new String[]{"/minetweaker blockinfo", "   Activates or deactivates block reader. In block info mode,", "   right-click a block to see ID, meta and tile entity data"}, new ICommandFunction() {
			...
			public void execute(String[] arguments, IPlayer player) {
				if(blockInfoPlayers.isEmpty()) {
					events.onPlayerInteract(LISTEN_BLOCK_INFO);
				}
	 ...
	 * @param utils		utils interface
	 */
	public static void init(IOreDict oreDict, IRecipeManager recipes, IFurnaceManager furnace, IGame game, ILoadedMods mods, IFormatter formatter, IVanilla vanilla, IUtilities utils) {
	...
	private static class ListenPlayerLoggedIn implements PlayerLoggedInHandler {
	...
	private static class ListenPlayerLoggedOut implements PlayerLoggedOutHandler {
	...
	private static class ListenBlockInfo implements PlayerInteractHandler {
