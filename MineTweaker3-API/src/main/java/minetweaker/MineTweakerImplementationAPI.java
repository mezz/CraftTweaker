...
public class MineTweakerImplementationAPI {
	...
	static {
		...
		minetweakerCommands.put("reload", new MineTweakerCommand("reload", new String[]{"/minetweaker reload", "    Reloads all scripts"}, (String[] arguments, IPlayer player) -> {
			MineTweakerAPI.logInfo("RELOADING SCRIPTS!");
			reloading = true;
			reload();
			reloading = false;
...
//		MineTweakerAPI.tweaker.rollback();
		if(MineTweakerAPI.server != null) {
			if(!MineTweakerAPI.server.isCommandAdded("minetweaker")) {
				server.addCommand("minetweaker", "", new String[]{"mt"}, (arguments, player) -> {
					if(arguments.length == 0) {
				...
				}, player -> server.isOp(player), null);
			}
		}
